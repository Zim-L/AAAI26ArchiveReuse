...
import core.AbstractGeneticAlgorithm;
...
import operators.MergeNonDominatedSortRanking;
...
public class SMSEMOARA<S extends Solution<?>> extends AbstractGeneticAlgorithm<S, List<S>> implements ZAlgorithm<S> {
	...
	private String name = "SMSEMOA-RA";
	 ...
	 * Constructor
	 */
	public SMSEMOARA(Problem<S> problem, int maxEvaluations, int populationSize, double offset,
	...
	protected List<S> selection(List<S> population) {
		...
		for (int i = 0; i < 2; i++) {
			S solution = selection.execute(JMetalRandom.getInstance().nextDouble()>0.5? population : archive.getSolutionList());
