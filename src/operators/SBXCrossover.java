      ...
      for (i = 0; i < parent1.variables().size(); i++) {
        ...
        if (randomGenerator.getRandomValue() <= 0.5) {
          if (Math.abs(valueX1 - valueX2) > EPS) {
            ...
            alpha = 2.0 - FastMath.pow(beta, -(distributionIndex + 1.0));

            if (rand <= (1.0 / alpha)) {
              betaq = FastMath.pow(rand * alpha, (1.0 / (distributionIndex + 1.0)));
            } else {
              betaq = FastMath.pow(1.0 / (2.0 - rand * alpha), 1.0 / (distributionIndex + 1.0));
            }
