...
import core.DoubleProblem;
import core.DoubleSolution;
import core.FastMath;
import core.RepairDoubleSolution;
import core.RepairDoubleSolutionWithBoundValue;
import core.Bounds;
import core.Check;
    ...
    for (int i = 0; i < x.length; i++) {
      if (randomGenerator.getRandomValue() <= mutationProbability) {
        ...
        if (yl == yu) {
          ...
          if (rnd <= 0.5) {
            xy = 1.0 - delta1;
            val = 2.0 * rnd + (1.0 - 2.0 * rnd) * (FastMath.pow(xy, distributionIndex + 1.0));
            deltaq = FastMath.pow(val, mutPow) - 1.0;
          } else {
            xy = 1.0 - delta2;
            val = 2.0 * (1.0 - rnd) + 2.0 * (rnd - 0.5) * (FastMath.pow(xy, distributionIndex + 1.0));
            deltaq = 1.0 - FastMath.pow(val, mutPow);
          }
          y = y + deltaq * (yu - yl);
