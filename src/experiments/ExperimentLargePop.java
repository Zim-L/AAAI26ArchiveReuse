...
public class ExperimentLargePop {
	...
	public static void main(String[] args)  {
		// population size = 1000, only SMS-EMOA
		maxEval = 10000000;
		populationSize = 1000;
		if (debug) {
			experimentBaseDirectory = "D:/Experiments/pop1000/";
	...
	public static void runExperiment(Problem problem, int run) {
		...
		SMSEMOA alg3 = createSMSEMOAL(problem);
		
		ArrayList<Double> hv3 = new ArrayList<Double>();
		...
		Consumer<ZAlgorithm> monitor = alg -> {
			if (alg.getT() < 10 || (alg.getEvaluations() >= 10 && alg.getEvaluations() < 100 && alg.getEvaluations()%10==0) 
				...
				if (alg.getName().contentEquals("SMSEMOA-L")) {
					hv3.add(calculateHypervolumeMin(alg.getPopulation(), ref.get(problem.getName())));
				}
		...
		alg3.setMonitor(monitor);
		t0 = System.currentTimeMillis();
		alg3.run();
		duration = System.currentTimeMillis() - t0;
		saveResult(alg3, duration, run);
		saveList(hv3, experimentBaseDirectory+problem.getName()+"/"+alg3.getName()+"/hv-"+run+".txt");
	...
	public static SMSEMOA createSMSEMOAL(Problem problem) {
